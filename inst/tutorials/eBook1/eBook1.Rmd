---
title: "eBook Getting Started with dartRverse" 
...
description: "This is the beginning of your journey learning the basics of dartRverse, a hands on guide."
---
...
The following materials accompany the [eBook: Getting Started with dartRverse](http://dartr.biomatix.org/dartR). 

The Worked Examples and Exercises that accompany the eBook are written in [Markdown](https://www.markdownguide.org/) and rendered in [learnr](https://rstudio.github.io/learnr/). This maintains currency of the eBook during the continual development of dartRverse. 
...
Having set the learning objectives and covered the theory in the early Sessions of each Chapter, the eBook links to the relevant Worked Examples listed in the main menu. In Chapter 1, the RStudio Refresher, simply work through the materials in RStudio. Chapter 2 on R as a programming language is optional. R programming is not required to use dartRverse. Depending upon your level of experience, it is fine to simply work through the Worked Examples 2-1 to 2-4 as it appears in the RStudio Tutorial Window by running the code in each RCode Box. This is designed to provide an overview.
...
All aspects of the material are beneficial for learning -- the eBook for an introduction to the theory and methods, the Worked Examples to gain experience with dartRverse functions and how they are combined in a workflow, and finally the Exercises to apply everything you have learned without coaching in a problem-solving context.

<small>If you can't see the left sidebar, you just need to make the tutorial panel wider.</small>

### Data

You should already have the required datafiles in your working directory, if you have been following the eBook instructions in Chapter 1. If the datafiles are not there, run

source("http://georges.biomatix.org/storage/app/media/eBook1/eBook_data_download.txt")

### Let's get started! 

Pick your Worked Example or Exercise from the Menu under direction from the eBook.

<p>**Good luck on your dartRverse journey!** 😁</p>
...
CRAN is the main public repository for the R ecosystem, a bit like the central library or app store for R packages, though with more formal checking. Packages submitted to CRAN are tested for installation, portability, and policy compliance before they are accepted. To use CRAN packages, you need to
...
and test that it has been installed.
...
- Install [Rtools]("https://cran.r-project.org/bin/windows/Rtools/rtools45/files/rtools45-6691-6492.exe") outside RStudio by downloading and executing the executable.

### Bioconductor {data-continue-text="Continue"}

Bioconductor is a specialized repository for bioinformatics and genomics. To download a package from Bioconductor, you need to
...
- If the package you seek is available from BioConductor, proceed as follows:
...
This material is **optional**. The objective is to introduce you to R programming. The package dartRverse does not require an in-depth knowledge of R programming, so those not wishing to dive deeper into the R programming aspects of SNP data analysis can skip this chapter and move directly to Chapter 3.
...
<a href="http://georges.biomatix.org/storage/app/media/eBook1/AI_Guide_to_R_vectors.htm"
...
For a more detailed account of working with factors in R, refer to the <a href="http://georges.biomatix.org/storage/app/media/eBook1/AI_Guide_to_R_Factors.htm"
...
<a href="http://georges.biomatix.org/storage/app/media/eBook1/AI_Guide_to_R_Matrices.htm"
   target="_blank" rel="noopener">AI Guide to R Matrices</a> and the <a href="http://georges.biomatix.org/storage/app/media/eBook1/AI_Guide_to_R_Arrays.htm"
...
```{r eval =FALSE}
utils::download.file(url = "http://georges.biomatix.org/storage/app/media/eBook1/Caretto.dat", destfile = "Caretto.dat")
```
...
df <- read.table("http://georges.biomatix.org/storage/app/media/eBook1/Caretto.dat")
```
...
```{r datatab2, echo = FALSE, cache=FALSE}
df <- read.table("http://georges.biomatix.org/storage/app/media/eBook1/Caretto.dat")
names(df) <- c("identity","sex","len","hw","wt")
...
For a more detailed account of working with dataframes in R, refer to the <a href="http://georges.biomatix.org/storage/app/media/eBook1/AI_Guide_to_R_Data_Frames.htm"
...
For a more detailed account of working with lists in R, refer to the <a href="http://georges.biomatix.org/storage/app/media/eBook1/AI_Guide_to_R_Lists.htm"
...
as this will attempt to assign the text string "NA" to element 4 in the numeric vector, coercing the whole vector to character (not the intended result).

The function `is.na()` is used to determine which values of a variable are missing and which have data. For example,

```{r}
weights <- c(3.5, 5.2, 7.8, NA, 6.7)
is.na(weights)
```
...
Download into your working directory, a csv file containing metadata for individuals to be used in later analyses. It is called [Tympo_ind_metadata.csv]("http://georges.biomatix.org/storage/app/media/eBook1/Tympo_ind_metadata.csv").
...
metadata <- read.csv("http://georges.biomatix.org/storage/app/media/eBook1/Tympo_ind_metadata.csv")
```
...
df <- read.csv("http://georges.biomatix.org/storage/app/media/eBook1/Tympo_ind_metadata.csv")
```
...
For a more detailed account of working with **apply** family functions in R, refer to the <a href="http://georges.biomatix.org/storage/app/media/eBook1/AI_Guide_to_R_apply_Family%20_of%20_Functions.htm"
...
<a href="http://georges.biomatix.org/storage/app/media/eBook1/AI_Guide_to_R_Functions.htm"
   target="_blank" rel="noopener">AI Guide to R functions</a>

### Winding up {data-continue-text="Next Topic"}

Got the hang of R? That is great, but remember, you do not need to be proficient in R to use dartRverse. 
...
The objective of this chapter is to introduce you to the manner in which dartRverse stores genotype data and associated metadata and how in input and export data into those datastructures from dartRverse or other data sources (e.g. vcf). 

This is the first substantive Chapter on dartRverse, so Worked Examples should be undertaken within the RStudio environment, and in particular, the RStudio Editor. This will require you to progressively transfer the code from the Tutorial Window to your RStudio Editor to execute it. 
...
Two files should be located in your RStudio working directory.

- [Report_Dtym25-13579_SNP.csv](http://georges.biomatix.org/storage/app/media/eBook1/Report_DTym25-13579_SNP.csv) : This is the set of SNP data for Canberra Grassland Earless Dragons in 2-row format as would be supplied by DArT.

- [Tympo_ind_metadata.csv](http://georges.biomatix.org/storage/app/media/eBook1/Tympo_ind_metadata.csv) : This file contains the additional individual metadata -- attributes assigned to each individual.
...
The next steps are to read the data into dartRverse, verify that the contents are as expected. 

### Read the data into dartRverse
...
- Open the file [sample_data_1Row.csv](http://georges.biomatix.org/storage/app/media/eBook1/sample_data_1Row.csv) in Excel. This is a set of SNP data for *Emydura*, a freshwater turtle,  in 1-row format as would be supplied by DArT.
...
- Now examine the individual metadata in the file [sample_metadata.csv](http://georges.biomatix.org/storage/app/media/eBook1/sample_metadata.csv). Note the two mandatory columns [id]{style="color:blue;"} and [pop]{style="color:blue;"}.
...
- Open the file [sample_data_SilicoDArT.csv](http://georges.biomatix.org/storage/app/media/eBook1/sample_data_SilicoDArT.csv) in Excel. This is a set of marker presence/absence data for *Cherax destructor* provided in SilicoDArT format by DArT.
...
These exercises bring you to the end of Chapter 3, the first substantive Chapter on dartRverse. 
...
### Read the data into dartRverse {data-continue-text="Continue"}
...
This will render the dartR genlight object compliant with dartRverse. This is particularly important if you have generated your data outside the DArT environment.
...
At this point you should have a good grasp on how to interrogate your dataset using the dartRverse report functions. 
...
It is not a fatal error to forget to recalculate the locus metrics because dartRverse scripts will detect if they have not been recalculated and rectify this before they a particular locus metric is needed. 


### Filter Loci on Secondaries

Sequence tags can contain more than one callable SNP marker. Because of their close proximity, these multiple SNPs within a single sequence tag (referred to in dartRverse as 'secondaries' are likely to be strongly linked (inherited together). This is problematic for many analyses, so one might wish to filter out the multiple SNPs to leave only one per sequence tag.
...
Let us apply a filtering regime to the unfiltered data. I have set verbosity to zero to limit the output

```{r filter, eval = FALSE}

gl <- gl.filter.sexlinked(gl, 'xy')
...
That is PCA in a nutshell. There is much more to it, and this will be covered in the next eBook on Advanced Topics in dartRverse.
...
#### Begin by reading the SNP data into dartRverse 

-	First, examine the contents of the metadata file, [Macrobrachium_ind_metadata.csv](http://georges.biomatix.org/storage/app/media/eBook1/Macrobrachium_ind_metadata.csv). This file has the individual metadata associated with each individual, including such attributes as sex, latitude, longitude, river etc. Recall that these individual metadata are assigned to each individual as the data are read in.

-	Read the data into dartRverse from [Report-DShr16-2215_2Row_SNP.csv]{style="color: blue;"}. Be sure to associate the individual metadata.
...
-	The authors of this work filtered on a CallRate threshold of 80% and on MAF threshold of 5%. They retained only 1,814 SNP loci. Repeat their filtering regime with dartRverse. Do you obtain the same result?
...
-	Read the data into dartRverse from [Report_DFwt25-10896_SNP_2Row.csv]{style="color:blue;"}. Be sure to associate the individual metadata.
...
This work has been published in [Conservation Biology](https://doi.org/10.1111/cobi.13989). You are encouraged to read this paper to gain a greater appreciation of the context of the analysis.
You are asked to use this dataset to apply what you have learned from this introductory eBook, drawing from across all Chapters of this eBook. Note that the data may have been modified for educational purposes and should not be used for research purposes.

#### Begin by reading the SNP data into dartRverse 

-	First, examine the contents of the metadata file, [Dasyurus_hallucatus_ind.metadata.csv](http://georges.biomatix.org/storage/app/media/eBook1/Dasyurus_hallucatus_ind.metadata.csv). This file has the metadata associated with each individual, including such attributes as sex, species, latitude, longitude, etc. 

-	We have already read the data into dartRverse from [Report_DDasy19-4717_1Row.csv]{style="color:blue;"} and saved it as [Dasyurus_SNP_raw.Rdata]{style="color:blue;"}. Load this binary datafile [Dasyurus_SNP_raw.Rdata]{style="color:blue;"} into dartRverse.
...
---
title: "eBook Getting Started with dartRverse" 
author: "The dartRverse Team"
date: | 
    ![](images/dartR7.png){width=6cm}
    
tutorial:
  id: "dartr.getting-started"   # pick any stable, unique string
  version: "1.0.23"                 # bump this to force a “fresh” tutorial state
...
Macrobrachium is a large and cosmopolitan crustacean genus of high economic importance worldwide. The authors of [this study](http://georges.biomatix.org/storage/app/media/eBook1/2019_Makombu_Ecology_Evolution_Macrobrachium.pdf) investigated the morphological and molecular identification of freshwater prawns of the genus Macrobrachium in South, Southwest and Littoral regions of Cameroon in Africa. Seven species occur in the area:  *M. vollenhovenii, M. macrobrachion, M. sollaudii, M. dux, M. chevalieri, M. felicinum* and an undescribed *Macrobrachium* species. The objective of the study was to validate (or invalidate) the identified species based on their genetic profiles. A total of 93 individuals representing these species were subjected to genetic characterization using 1,814 DArT markers. This study is considered valuable for informing breeding design and genetic resource conservation programs for Macrobrachium in Africa.

![](images/ex7-1.jpg)

Refer to the [original paper](http://georges.biomatix.org/storage/app/media/eBook1/2019_Makombu_Ecology_Evolution_Macrobrachium.pdf) by Judith Makombu and her team for further detail on the study. 
...
This work has been published in [Conservation Biology](http://georges.biomatix.org/storage/app/media/eBook1/2023_Shaw_Quolls_Conservation_Biology.pdf). You are encouraged to read this paper to gain a greater appreciation of the context of the analysis.
You are asked to use this dataset to apply what you have learned from this introductory eBook, drawing from across all Chapters of this eBook. Note that the data may have been modified for educational purposes and should not be used for research purposes.
